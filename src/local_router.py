...
2) open_app — 仅适合「打开、启动、切换到某个 Mac 应用」（如 Safari、微信、计算器、终端）。
3) send_wechat — 仅适合「给某人发微信」或「用微信发消息给某人」。
4) fallback — 复杂任务、需要搜索/看图/写代码/系统设置/多步操作/不确定时一律选它。
...
open_app 的 app 字段只用英文小写别名：safari, chrome, wechat, calculator, terminal, notes, mail, music, 或 weixin（等同 wechat）。
...
def _normalize_route(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if skill == "open_app":
        ...
        if not app:
            return {"skill": "fallback"}
        return {"skill": "open_app", "args": {"app": str(app).strip().lower()}}
